...
class SoundscapeComposer:
    ...
    V7: Thicker Mix (Warmth), Modulated Delays (Tape Wobble), Dual Convolution.
    """
    ...
    def _get_random_fx_chain(self, layer_type):
        """Generates a random FX chain based on layer type."""
        chain = []
        ...
        if layer_type == 'drone':
            ...
            if random.random() > 0.7:
                chain.append(Phaser(rate_hz=random.uniform(0.05, 0.2), depth=random.uniform(0.3, 0.6), mix=random.uniform(0.1, 0.4)))
        
        elif layer_type == 'body':
            if random.random() > 0.5:
                chain.append(Chorus(rate_hz=random.uniform(0.1, 0.5), depth=random.uniform(0.2, 0.5), mix=random.uniform(0.3, 0.6)))
            if random.random() > 0.5:
                # V7: Widen Delay Range (0.1s to 1.2s)
        ...
        # 2. EQ / Filtering (V7: Thicker Mix)
        if layer_type == 'drone':
            chain.append(LowpassFilter(cutoff_frequency_hz=random.uniform(300, 800)))
        elif layer_type == 'body':
            chain.append(HighpassFilter(cutoff_frequency_hz=random.uniform(80, 200))) # Warmth preserved
            chain.append(LowpassFilter(cutoff_frequency_hz=random.uniform(1500, 4000)))
        elif layer_type == 'texture':
            chain.append(HighpassFilter(cutoff_frequency_hz=random.uniform(600, 1500)))

        # 3. Per-Layer Convolution (Depth)
    ...
    def compose_pulse_layer(self, input_file, duration_minutes=10):
        ...
        # V7: Modulated Pulse Delay
        board = Pedalboard([
            HighpassFilter(cutoff_frequency_hz=random.uniform(300, 600)),
            Phaser(rate_hz=random.uniform(0.1, 0.4), depth=0.6, mix=0.4),
            # Wobbly Delay
            Delay(delay_seconds=random.uniform(0.2, 0.5), feedback=0.4, mix=0.3),
            Chorus(rate_hz=0.8, depth=0.3, mix=0.2), 
